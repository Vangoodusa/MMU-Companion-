...
class TrackFormCompletionUseCase @Inject constructor(
    ...
    private suspend fun generateAutomaticReport(form: Form, userId: String) {
        try {
            val reportType = when (form.type) {
            ...
            val request = ReportGenerationRequest(
                ...
                reportTitle = "Auto-generated ${reportType.displayName} for ${form.type.displayName}",
                format = ExportFormat.PDF,
                formIds = listOf(form.id),
                parameters = mapOf(
                    "autoGenerated" to true,
                    "triggerFormId" to form.id,
                    "triggerFormType" to form.type.name
                )
            )
